...
def farm_loop(client, do_upgrade=True, claim_interval=600):
    BURST_TAPS = 1_000_000
    TAPS_PER_LOOP = 100_000
    ...
    print("🚀 Auto tap dimulai")
    print(f"⚡ Batch: {TAPS_PER_LOOP:,} tap/loop")
    print(f"🎯 Burst: {BURST_TAPS:,} tap")

    while True:
        # Tambahkan tap dalam jumlah besar sekaligus
        tap_count += TAPS_PER_LOOP
        total_unclaimed_taps += TAPS_PER_LOOP

        # Burst mencapai 1 juta → mulai burst baru
        if tap_count >= BURST_TAPS:
            print(f"🔥 Burst {BURST_TAPS:,} tap tercapai!")
            tap_count = 0
        ...
        if now - last_status >= 10:
            print(
                f"⚡ Burst: {tap_count:,} | "
                f"Unclaimed: {total_unclaimed_taps:,}"
            )
            last_status = now
        ...
        if now - last_claim >= claim_interval:
            if total_unclaimed_taps > 0:
                print(
                    f"💰 Claim {total_unclaimed_taps:,} tap..."
                )

                try:
                    result = client.claim_taps(total_unclaimed_taps)
