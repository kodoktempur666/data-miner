...
def farm_loop(
    ...
    claim_interval: int = 600,
) -> None:
    """
    Continuous burst tapping.

    - Burst sampai 1.000.000 tap
    - Setelah 1.000.000 tap, counter burst di-reset
    - Burst berikutnya langsung dimulai
    - Claim dilakukan setiap 10 menit
    """

    BURST_TAPS = 1_000_000

    tap_count = 0
    total_unclaimed_taps = 0

    last_claim = time.monotonic()
    last_status = time.monotonic()

    log.info(
        "starting burst auto-tap | burst=%s taps | claim every %s seconds",
        f"{BURST_TAPS:,}",
        claim_interval,
    )
    ...
    while True:
        try:
            ...
            tap_count += 1
            total_unclaimed_taps += 1

            # Jika mencapai 1 juta tap,
            # reset counter burst dan mulai burst berikutnya.
            if tap_count >= BURST_TAPS:
                log.info(
                    "burst reached %s taps — starting next burst",
                    f"{BURST_TAPS:,}",
                )
            ...
            if now - last_status >= 10:
                amount = total_unclaimed_taps * client.TAP_VALUE
                remaining = max(
                    0,
                    claim_interval - (now - last_claim),
                )

                log.info(
                    "burst=%s/%s | unclaimed=%s taps | %.6f COIN | claim in %.0fs",
                    f"{tap_count:,}",
                    f"{BURST_TAPS:,}",
                    f"{total_unclaimed_taps:,}",
                    amount,
                    remaining,
                )
            ...
            # ========================================================= #
            # CLAIM SETIAP 10 MENIT
            # ========================================================= #

            if now - last_claim >= claim_interval:

                if total_unclaimed_taps > 0:

                    claim_taps = total_unclaimed_taps

                    amount = round(
                        claim_taps * client.TAP_VALUE,
                        6,
                    )

                    log.info(
                        "10 minutes reached — claiming %s taps = %.6f COIN",
                        f"{claim_taps:,}",
                        amount,
                    )

                    res = client.claim_taps(claim_taps)

                    log.info(
                        "claim result: %s | taps=%s | amount=%.6f",
                        res.get("message", "ok"),
                        f"{claim_taps:,}",
                        amount,
                    )

                    # Reset jumlah yang sudah di-claim.
                    total_unclaimed_taps = 0

                else:
                    log.info("10 minutes reached — nothing to claim")
                ...
                if do_upgrade:
                    bal = float(
                        user.get("miningBalance") or 0
                    )

                    if bal > 0:
                        upgrade = client.upgrade_level()

                        log.info(
                            "upgrade-level: %s",
                            upgrade.get("message", "ok"),
                        )
            ...
            log.exception(
                "unexpected error: %s — retry in 10s",
                exc,
            )
            time.sleep(10)
# --------------------------------------------------------------------------- #
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.run:
        client = asyncio.run(
            ensure_auth(_load_cached())
        )

        farm_loop(
            client,
            do_upgrade=not args.no_upgrade,
            claim_interval=600,  # 10 menit
        )
